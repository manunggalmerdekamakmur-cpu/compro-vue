...
const initLazyLoader = () => {
  if ('IntersectionObserver' in window) {
    ...
    window.App.lazyLoader = {
      ...
      scan: () => document.querySelectorAll('img[data-src]').forEach(img => imageObserver.observe(img))
    }
  }
}
...
const loadFontAwesome = async () => {
  ...
  } catch (e) { console.warn('Font Awesome failed to load:', e) }
}
...
const initApp = async () => {
  ...
  try {
    ...
    app.directive('lazy', {
      mounted(el) { window.App?.lazyLoader?.observe(el) }
    })
